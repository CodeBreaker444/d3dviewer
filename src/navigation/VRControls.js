...
class FlyMode {
  ...
  update(vrControls, delta) {
    ...
    let sceneDir = toScene(vrPos.clone().add(vrDir), vrControls.node).sub(
      scenePos
    );
...
class RotScaleMode {
  ...
  update(vrControls, delta) {
    ...
    let mToOrigin = new THREE.Matrix4().makeTranslation(
      ...toScene(start_center, this.startState).multiplyScalar(-1).toArray()
    );
    let mToStart = new THREE.Matrix4().makeTranslation(
      ...toScene(start_center, this.startState).toArray()
    );
...

    {
      ...
      let sceneDir = toScene(vrPos.clone().add(vrDir), this.startState).sub(
        scenePos
      );
      sceneDir.normalize().multiplyScalar(scale);
      let sceneTarget = scenePos.clone().add(sceneDir);
...
export class VRControls extends EventDispatcher {
  ...
  createInfo() {
    let texture = new THREE.TextureLoader().load(
      `${Potree.resourcePath}/images/vr_controller_help.jpg`
    );
  ...
  onStart() {
    ...
    const texture = loader.load([
      `${Potree.resourcePath}/textures/skybox2/px.jpg`,
      `${Potree.resourcePath}/textures/skybox2/nx.jpg`,
      `${Potree.resourcePath}/textures/skybox2/py.jpg`,
      `${Potree.resourcePath}/textures/skybox2/ny.jpg`,
      `${Potree.resourcePath}/textures/skybox2/pz.jpg`,
      `${Potree.resourcePath}/textures/skybox2/nz.jpg`,
    ]);
