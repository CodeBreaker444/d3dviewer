...
export class Utils {
  ...
  static linearGradientBackgroundTexture(width, height) {
    // Linear gradient function for the background
    function createLinearGradient(context, width, height) {
      const gradient = context.createLinearGradient(0, 0, 0, height);
      gradient.addColorStop(0, 'rgba(0, 70, 110, 0.5)'); // Top color
      gradient.addColorStop(1, 'rgba(0, 10, 30, 0.5)'); // Bottom color
      return gradient;
    }

    // Create canvas for drawing the gradient
    const canvas = document.createElement('canvas');
    const context = canvas.getContext('2d');
    canvas.width = width;
    canvas.height = height;

    // Apply the linear gradient to the canvas
    const gradient = createLinearGradient(context, width, height);
  ...
  static pixelsArrayToCanvas(pixels, width, height) {
    ...
    for (let i = 0; i < parseInt(height / 2); i++) {
      ...
      let lineI = pixels.slice(
        i * bytesPerLine,
        i * bytesPerLine + bytesPerLine
      );
      let lineJ = pixels.slice(
        j * bytesPerLine,
  ...
  static getNorthVec(p1, distance, projection) {
    if (projection) {
      ...
      const polarRadius = Math.sqrt(
        (llP2[0] - llP1[0]) ** 2 + (llP2[1] - llP1[1]) ** 2
      );
